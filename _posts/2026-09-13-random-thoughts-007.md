...
Concept Review 真正改变的是 Review 的基本单位：**Code Review 关心哪些代码发生了变化，Concept Review 关心这次修改让系统中的概念、关系和边界如何演进。**

这并不意味着 Reviewer 要自己从几千行代码中推导答案，否则它仍然只是换了名字的 Code Review。AI 在完成实现以后，还应该把代码变化重新翻译成人能够理解的概念演进：哪些领域概念被新增、修改或者移除，哪些数据关系、业务规则和模块边界受到影响。

在开发之前，人和 AI 先明确预期的概念演进；开发完成以后，AI 再从实际代码中梳理出真正发生的概念演进，由 Reviewer 判断两者是否一致。

AI 可以同时成为实现者和解释者，但最终的判断依然由人完成。Reviewer 不需要理解每一行代码，却必须理解系统正在如何演进。

## 把系统复杂度控制在人的理解范围之内

Concept Review 最重要的目的，不只是提高 Review 效率，而是把整个系统的复杂度控制在团队的理解范围之内。
